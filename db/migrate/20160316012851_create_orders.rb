class CreateOrders < ActiveRecord::Migration
  def change
    create_table :orders do |t|
      ...
      t.decimal :total_price, :decimal, :precision => 10, :scale => 2
      t.string :currency, limit: 3		# EUR, USD, CAD, PLZ, RUR, etc.
      t.decimal :total_discount, :decimal, :precision => 10, :scale => 2
      t.text :comment

      t.timestamps null: false

      t.check "total_quantity > 0", name: 'order_quantity_chk'
      t.check "total_price > 0", name: 'order_price_chk'
      t.check "total_discount > 0", name: 'order_discount_chk'
      t.check "total_price > total_discount", name: 'order_price_gt_discount_chk'
    end
